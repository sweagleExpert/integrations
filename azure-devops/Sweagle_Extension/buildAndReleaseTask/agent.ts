...
import { HttpsProxyAgentOptions } from './HttpsProxyAgent';
...
export default class HttpsProxyAgent extends Agent {
	...
	constructor(_opts: string | HttpsProxyAgentOptions) {
		...
		console.log('creating new HttpsProxyAgent instance: %o', opts);
		super(opts);
	...
	async callback(
		...
		if (secureProxy) {
			console.log('Creating `tls.Socket`: %o', proxy);
			socket = tls.connect(proxy as tls.ConnectionOptions);
		} else {
			console.log('Creating `net.Socket`: %o', proxy);
		...
		if (statusCode === 200) {
			...
			if (opts.secureEndpoint) {
				...
				console.log('Upgrading socket connection to TLS');
		...
		req.once('socket', (s: net.Socket) => {
			console.log('replaying proxy buffer for failed request');
