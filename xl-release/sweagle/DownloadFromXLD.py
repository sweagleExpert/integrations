...
try:
	...
	try:
		...
		raise Exception("\r get dictionaries error : " + str(ukhe) + "\n")

	if response.getStatus() != 200:
		raise Exception("\r get dictionaries error. Status = [" + str(response.getStatus()) + "] Error : " + response.getResponse())
	#End if

	#For DEBUG print ("\r - XLD response: " + response.getResponse())
	...
	for lxlddictionary in lxlddictionaries:
        ...
        #For DEBUG print("\r - Dictionary :" + response.getResponse())
        ...
        # BEGIN: This is to use if you want SWEAGLE to contain only XLD dictionary key/values
        #json_dict = json.loads(r.getResponse())
        #data = json_dict['entries']
        # END: This is to use if you want SWEAGLE to contain only XLD dictionary key/values
        # BEGIN: This is to use if you want SWEAGLE to contain the full XLD dictionary object
		data = response.getResponse()
        # END: This is to use if you want SWEAGLE to contain the full XLD dictionary object
